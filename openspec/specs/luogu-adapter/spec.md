# luogu-adapter Specification

## Purpose

定义 `@oj-agent/core` 中洛谷平台适配器 `LuoguAdapter` 的契约,包括基于 `lentille-context` 的列表/题面解析、CSRF 提交流程、判题轮询与降级策略,以及与 [[platform-adapter]] / [[http-client]] 的集成约束。
...
#### Scenario: 列表解析
- **WHEN** ���用 `adapter.listProblems({ page: 1 })`
