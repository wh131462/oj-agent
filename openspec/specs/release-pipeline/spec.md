...
定义 oj-agent 仓库的统一发布流水线,包括 `pnpm release` 编排脚本、VSCode 扩展打包对 `workspace:*` 依赖的处理、dry-run 校验与各包版本号一致性约束。
...
release 脚本 SHALL 保证 `@oj-agent/core`、`@oj-agent/cli` 与 `oj-agent` 扩展三者版本号��同一次发布中显式声明,任一缺失或冲突 MUST 阻止发布。
