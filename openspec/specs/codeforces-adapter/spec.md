# codeforces-adapter Specification

## Purpose

定义 `@oj-agent/core` 中 Codeforces 平台适配器 `CodeforcesAdapter` 的契约,包括题目列表来源(官方 API)、题面抓取(登录会话 + HTML 解析)、提交与轮询行为,以及与 [[platform-adapter]] / [[http-client]] 的集成约束。
